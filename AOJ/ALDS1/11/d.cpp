...
void dfs(int idx, VEC2<int> &G, VEC<int> &color, int c) {
  if(color[idx] != 0) return;  // 探索済みならスルー

  color[idx] = c;  // 到達済み、探索未完了

  REP(i, 0, G[idx].size()) {
    int next = G[idx][i];
    if(color[next] != 0) continue;  // 隣接頂点が探索済みならスルー

    if(color[next] == 0) {     // 隣接頂点が未到達なら
      dfs(next, G, color, c);  // 未到達の隣接頂点を探索
    }
  }
}
...
void solve() {
  ...
  REP(i, 0, n) { dfs(i, G, color, ++c); }

  int q;
  cin >> q;
  REP(i, 0, q) {
    cin >> s >> t;
    if(color[s] == color[t]) CO("yes");
    else
      CO("no");
  }
}
