...
void solve() {
  int h, w;
  int dir_x[3] = {-1, 0, 1}, dir_y[3] = {-1, 0, 1};
  cin >> h >> w;
  VEC<string> s(h);
  VEC2<char> ans(h, VEC<char>(w));
  REP(i, 0, h) cin >> s[i];
  ...
  REP(i, 0, h) {
    REP(j, 0, w) {
      ...
      REP(x, 0, 3) {
        if(j + dir_x[x] < 0 || j + dir_x[x] >= w) continue;
        REP(y, 0, 3) {
          if(i + dir_y[y] < 0 || i + dir_y[y] >= h) continue;

          if(s[i + dir_y[y]][j + dir_x[x]] == '#') cnt++;
        }
      }
      ans[i][j] = itoc(cnt);
      cnt = 0;
