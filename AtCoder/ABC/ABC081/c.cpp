...
void solve() {
  ...
  VEC<pair<int, int>> sorted;
  for(auto itr : dict) {
    sorted.emplace_back(make_pair(itr.second, itr.first));
  }
  sort(ALL(sorted));

  // REP(i, 0, sorted.size()) { cout << sorted[i].first << " " << sorted[i].second << en; }

  int ans = 0;
  REP(i, 0, sorted.size() - k) { ans += sorted[i].first; }
  CO(ans);
}
